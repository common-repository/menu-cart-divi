...
class LWP_MenuCart extends ET_Builder_Module {
	...
	public function get_custom_css_fields_config() {
		return array(
			'inner-module' => array(
				'label'    => esc_html__( 'Inner Module', 'lwp-divi-module' ),
				'selector' => '%%order_class%% .et_pb_module_inner',
			),
			'cart-link'    => array(
				'label'    => esc_html__( 'Cart Link', 'lwp-divi-module' ),
				'selector' => '%%order_class%% .lwp_cart_module',
			),
			'cart-icon'    => array(
				'label'    => esc_html__( 'Cart Icon', 'lwp-divi-module' ),
				'selector' => '%%order_class%% .lwp_cart_icon',
			),
			'cart-count'   => array(
				'label'    => esc_html__( 'Cart Count', 'lwp-divi-module' ),
				'selector' => '%%order_class%% .lwp_menu_cart_count',
			),
			'cart-sep'     => array(
				'label'    => esc_html__( 'Cart Separator', 'lwp-divi-module' ),
				'selector' => '%%order_class%% .lwp_menu_cart_sep',
			),
			'cart-price'   => array(
				'label'    => esc_html__( 'Cart Price', 'lwp-divi-module' ),
				'selector' => '%%order_class%% .woocommerce-Price-amount',
			),
		);
	}
	...
	public function render( $attrs, $content, $render_slug ) {
		...
		add_action( 'wp_footer', 'lwpdm_refresh_cart_on_page_load' );
		...
		if ( false == $options ) {  // Show default content
			...
			if ( isset( $options['hide_item_text'] ) && $options['hide_item_text'] == 'on' ) {
				$hide_item_text = 'on';
			}
		...
		if ( 'on' === $show_item_count && 'off' === $hide_item_text ) {
			...
			} elseif ( ! empty( $single_item_text ) && $total == 1 ) {
				$single    = $total . ' ' . $single_item_text;
				$cartcount = $single;
			} elseif ( ! empty( $multiple_item_text ) && ( $total == 0 || $total > 1 ) ) {
				$multiple  = $total . ' ' . $multiple_item_text;
				$cartcount = $multiple;
			} else {
